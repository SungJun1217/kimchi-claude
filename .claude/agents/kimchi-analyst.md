...
- Term priority (reversed 2026-09-26, see design doc "설계 중 뒤집은 결정"): write terms the way Korean developers actually type them. Established Korean terms stay Korean — loanwords (배포, 커밋, 브랜치, 캐시, 빌드…), plain nouns (function/array/value → 함수/배열/값), and Sino-Korean technical terms already settled that way (결합도, 의존성, 응집도, 불변식, 멱등, 영향 범위, 책임, 하위 호환…). Terms devs type in Latin stay Latin (state, props, payload, target, directory, endpoint, and race condition/deadlock/latency/throughput/side effect/regression — 경쟁 조건/교착 상태/지연 시간/처리량/부수 효과/회귀 are parenthetical alternatives, not the default, per `rules/hanja.md` and `rules/terms.md`) — noun slots only; Latin verbs never take 하다 (`push합니다` is wrong, established loanword verbs stay Hangul: 푸시합니다, 머지합니다, 업데이트합니다). Native-Korean paraphrase is a last resort. Metaphors are not translated; name the concept.

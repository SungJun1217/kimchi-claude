---
name: temper
description: Develop one unit of work end-to-end in kimchi-claude — feature branch off develop → analyze (if needed) → implement → review → real try-out (if runtime) → version bump + commit → merge into develop. Use when the user runs /temper <task>. Never pushes, never touches main.
...
- If it is clearly several independent units, propose the split and ask which to do first. One `/temper` = one commit = one version.
...
Otherwise spawn `kimchi-analyst` with the task and whatever you know. Use its conclusion,
...
Spawn `kimchi-implementer` with a concrete brief: files, intended behavior, invariants to watch,
...
Spawn `kimchi-reviewer` with one line of intent and the implementer's test result. Tell it the
...
## 3.5 Try it for real (runtime changes only)

When the change affects what happens in a user's session — hook decisions or messages
(`hooks/`), the generated output style, skill loading or content a skill points to, repo-language
inference — spawn `kimchi-tester` with the intent, the scenarios that prove it, and what "done"
looks like. Skip it for docs/tests/dev-tooling-only changes and pure rule-data additions already
pinned by tests, and say so in the report.
...
- BLOCKED scenarios (auth) and pre-existing problems it surfaces are not blockers for this
  commit — list them in the report as follow-ups.

## 4. Commit

- Bump the version in **both** `package.json` and `.claude-plugin/plugin.json` from the current
  `develop` version: new capability → minor, fix/doc → patch.
- Stage only the files this unit touched plus the two version files (`git add <paths>`, never `-A`).
- Subject: `0.x.y — <합니다체 한 문장>`, matching `git log --oneline -20`. Start from the
  implementer's suggestion. The body is Korean 합니다체: *why* the change was made first (the
  cause, usually a measured fact), then what changed, then what was deliberately left out and why,
  wrapped at ~80 columns. End with the attribution trailer from the current system instructions.
- The pre-commit gate (`.claude/hooks/pre-commit-gate.mjs`) may block the commit. Treat that as a
...
- The commit hash and subject (with the new version), and the `develop` merge commit
- Review rounds used, and any findings deliberately left unfixed
- Try-out result (or why it was skipped), plus follow-ups it surfaced

...
- CI (`.github/workflows/test.yml`) runs `npm test` on every push to `main`, `develop`, `feature/**` and on every pull request. `.github/workflows/codeql.yml` runs CodeQL (`javascript-typescript`, `build-mode: none`) on push/PR to `main`/`develop` and weekly on a schedule. `.github/workflows/kimchi-lint.yml` dogfoods this repo's own GitHub Action (`uses: ./`) on every PR, posting inline review comments for tone/PII findings in changed docs, commit messages, and PR text. Pushing a `v*` tag runs `.github/workflows/release.yml`, which checks that the tag and both version files agree, runs `npm test` again, creates the GitHub Release, and force-moves the floating `v0` tag to it (so `uses: SungJun1217/kimchi-claude@v0` always resolves to the latest 0.x release). The same tag push (and `workflow_dispatch`) runs `.github/workflows/docs.yml`, which stages README.md/docs/design.md/CONTRIBUTING.md/SECURITY.md/assets via `scripts/publish-docs.mjs` and publishes them to GitHub Pages and the wiki — the repo stays the source of truth; Pages and the wiki are never hand-edited. Dependabot (`.github/dependabot.yml`) opens weekly PRs against `develop` for GitHub Actions versions only (there are no npm dependencies); merge them like any unit, with a version bump.
- The GitHub Action (`action.yml`, `scripts/action-lint.mjs`, `scripts/build-review.mjs`) reuses `hooks/lib/artifact.mjs`'s `extractTargets` and the same lint/PII engines the hooks use — don't reimplement the tone/PII judgment for CI. The scanning scripts stay offline (invariant 10); `gh` CLI calls live only in `action.yml`'s shell steps.
- `.github/workflows/triage.yml` runs on `issues: [opened, edited]` when the issue carries the `false-positive` or `missed-expression` label: it checks out `main` explicitly (issue events have no ref of their own, and "current rules" means the latest release, not `develop`), reads the issue body from `GITHUB_EVENT_PATH` (never interpolated into shell), reproduces the reported sentence with `scripts/triage-report.mjs` (offline, reuses `scripts/action-lint.mjs`'s `findingsForTarget`/`findingsForText`; ignores `kimchi-allow-rrn` in the reported text since that opt-out is for maintainers' own docs, not untrusted issue submitters), and posts/updates a single bot comment (marker `<!-- kimchi-claude-triage -->`) while toggling the `reproduced`/`not-reproduced` labels (and `needs-info` when the sentence contained a resident registration number).

...
| ✏️ 표기와 맞춤법 | 빌드 타겟 디렉토리에 올릴께요<br>배포할때 확인해야합니다 | 빌드 타깃 디렉터리에 올릴게요<br>배포할 때 확인해야 합니다 |
<!-- kimchi-ignore-end -->
...
| 갈래 | 개수 | 누가 막나 |
|---|---|---|
| 치환 | 460 | 린터가 잡고, `KIMCHI_AUTOFIX=1`이면 바로 고칩니다 |
| 정규식 | 140 | 린터가 잡아서 알려 줍니다 |
| 프롬프트 | 109 | 문자열로는 못 잡아서 출력 스타일로만 막습니다 |
| **합계** | **709** | 그중 46개가 출력 스타일 본문에 들어갑니다 |
...
표기는 국립국어원 외래어 표기와 맞춤법을 따릅니다. 디렉터리, 타깃, 릴리스로 쓰고 할게요, 할 때로 씁니다.
다만 스냅샷, 컨텍스트, 리포지토리처럼 현장 표기가 굳은 말은 관행을 따릅니다. 그런 말의 목록과 근거는
[설계 문서](docs/design.md#국립국어원-자료를-들인-방법-v01319014)에 있습니다.

...
클로드 코드가 한국어로 답할 때 자주 나오는 어색함은 세 가지입니다.

<!-- kimchi-ignore-start -->
| 유형 | 예 | 이렇게 씁니다 |
|---|---|---|
| 🔤 직역 신조어 | `thin contract` → "얇은 계약" | 결합도를 낮춥니다 |
...
스킬로 뒀습니다. 스킬이 평소에 차지하는 자리는 설명 몇 줄, 약 330토큰입니다.
...
| 갈래 | 개수 | 누가 막나 |
|---|---|---|
| 치환 | 421 | 린터가 잡고, `KIMCHI_AUTOFIX=1`이면 바로 고칩니다 |
| 정규식 | 70 | 린터가 잡아서 알려 줍니다 |
| 프롬프트 | 102 | 문자열로는 못 잡아서 출력 스타일로만 막습니다 |
| **합계** | **593** | 그중 49개가 출력 스타일 본문에 들어갑니다 |
<!-- /kimchi:counts -->
...
통과해야 하고, `npm test`의 `dogfood` 시험이 확인합니다. 이 README도 예외가 아닙니다.

...
test("ext를 넘기면 문서 전용 가리개(들여쓰기 코드·<pre>/<code>)도 건드리지 않는다", () => {
  ...
  assert.equal(findParticleErrors(indented, { ext: "md" }).length, 0);
  assert.equal(findParticleErrors(indented).length, 1, "ext 없이는 여전히 잡아야(하위 호환)");

  const pre = "<pre>json를 출력</pre>";
  assert.equal(findParticleErrors(pre, { ext: "md" }).length, 0);

  const { text, applied } = fixParticles(indented, { ext: "md" });
